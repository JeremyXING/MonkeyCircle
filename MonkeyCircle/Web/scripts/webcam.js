...
window.webcam = {
	...
	swf_url: 'dev/pluginJs/camera/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
	shutter_url: 'dev/pluginJs/camera/shutter.mp3', // URI to shutter.mp3 sound
	...
	get_html: function(width, height, server_width, server_height) {
		...
		if(flashChecker()){
			...
			html += "<p class='alert alert-warning'>浏览器没有flash插件，<a href='http://www.adobe.com/go/getflashplayer' target='_blank'>请下载安装。</a></p>";
//			$("span-btn").html("<a class='btn btn-primary' href='user-challenge.action?retype="+$('#input-retype').val()+"&reid="+$('#input-reid').val()+"&order=0)>开始挑战</a>");
		}
	...
	get_movie: function() {
		...
		if (!this.loaded) return showInfo("ERROR: Movie is not loaded yet");
		var movie = document.getElementById('webcam_movie');
		if (!movie) showInfo("ERROR: Cannot locate movie 'webcam_movie' in DOM");
		return movie;
	},
	...
	set_shutter_sound: function(enabled, url) {
		...
		this.shutter_url = url ? url : 'dev/pluginJs/camera/shutter.mp3';
	},
...
//				aleshowInfolash_notify: " + type + ": " + msg);
				break;
		}
	}
};
function showInfo(msg){
	...
	$("#cam").html("<p class='alert alert-warning'>flash加载失败：请检查是否有摄像头，并且保证摄像头工作正常。</p>");
	$("#span-btn").html("<a class='btn btn-primary' href='coding-page.action?retype="+"reid="+$('#input-reid').val()+"'>开始挑战</a>");
}
